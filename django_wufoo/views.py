...
def sync_fields(wufoo_form):
    ...
    for wufoo_field in wufoo_form.fields:
        if wufoo_field.Choices:
            # Handle choices
            # Some fields, like the radio, dropdown and likert, represent a choice from several available choices.
            # The <Score> element only shows up in the likert field type.
            # see also http://help.wufoo.com/articles/en_US/SurveyMonkeyArticleType/The-Fields-API#choices
            choices = []
            choices_json = ""
            for choice in wufoo_field.Choices:
                choices_dict = {}
                choices_dict['label'] = choice.Label
                if choice.Score:
                    choices_dict['score'] = choice.Score
        ...
        if wufoo_field.Choices:
            # print "wufoo_field.Title: %s, choices_json: %s" % (wufoo_field.Title, choices_json)
            # update choices fields if they have changed
            field.choices = choices_json
            field.save()
